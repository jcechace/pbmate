...
### SDK Coverage Targets
Taskfile targets for local SDK code coverage measurement. `sdk:cover:unit` runs unit tests with per-function coverage report (49.6%). `sdk:cover` runs unit + integration tests, merges coverage profiles, and prints combined report (68.5%). Uses `-coverpkg=./...` so integration tests in `sdk/integtest/` count toward `sdk/v2` coverage. Coverage output files (`*.out`) already ignored by `.gitignore`.

### Write Operation Integration Tests
42 integration tests covering command dispatch, lock checking, and validation for all write operations across all 5 services. Tests verify the exact PBM command document inserted into `pbmCmd` — field mapping, types, nil/empty handling. Backup write tests: Start (logical with full field mapping, physical, incremental base/non-base, default compression), Cancel, Delete (by name, before with type/profile filters, older-than with duration, zero-duration), CanDelete (success, not-found, in-progress, non-chain-base). Restore write tests: Start (snapshot with all optional fields, PITR with oplog timestamp, physical/incremental unwaitable results, minimal), result type detection (waitable vs unwaitable). Config write tests: SetProfile (command dispatch with profile name), RemoveProfile, Resync (main, main with include-restores, profile with clear, all profiles). PITR write tests: Delete (before absolute cutoff, older-than relative duration, zero-duration delete-all). Lock tests: ConcurrentOperationError blocking Start/Delete/Resync/PITR-Delete, stale lock passthrough, Cancel bypasses lock. Validation tests: 9 tests verifying no command dispatched on invalid input (UsersAndRoles with specific collections, missing backup name, missing PITR target, empty delete name, empty profile name, future dates, negative duration).
